...
log_lik <- function(alpha, beta, gamma, csi) {
  ...
  log_lik_k <- function(k) { 
    ...
    lambda_k <- function(t) {
      log(lambda_xy(t))+gamma+alpha[i[k]]+beta[j[k]]+csi[1]*t
    }
    
    mu_k <- function(t) {
      log(mu_xy(t))+alpha[j[k]]+beta[i[k]]+csi[2]*t
    }
...
mod_2_CVXR = c(solution$getValue(alpha), solution$getValue(beta), solution$getValue(gamma), solution$getValue(csi))
save(mod_2_CVXR, file = "sol/mod_2_CVXR.RData")
mod_2_CVXR_lik = solution$value
save(mod_2_CVXR_lik, file = "sol/mod_2_CVXR_lik.RData")
