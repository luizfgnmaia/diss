...
pred_mod_3 <- function(n = 10000, home_team, away_team, score_home = 0, score_away = 0, minute = 0, half = 1) {
  
  pred <- function(home_team, away_team, score_home, score_away, minute, half) {
    
    if(half == 1) { # Tempo regulamentar do primeiro tempo
      while(minute < 45) {
        lambda_xy = ifelse(score_home > score_away, mod_3$lambda_xy["10"],
                           ifelse(score_away < score_home, mod_3$lambda_xy["01"],
                                  0))
        mu_xy = ifelse(score_home > score_away, mod_3$mu_xy["10"],
                       ifelse(score_away < score_home, mod_3$mu_xy["01"],
                              0))
      ...
      U1 = rpois(1, lambda = mod_3$eta[1] + mod_3$phi[1]*(score_home + score_away))
      u1 = 0
      while(u1 < U1) {
        lambda_xy = ifelse(score_home > score_away, mod_3$lambda_xy["10"],
                           ifelse(score_away < score_home, mod_3$lambda_xy["01"],
    ...
    while(minute < 45) {
      ...
      mu_xy = ifelse(score_home > score_away, mod_3$mu_xy["10"],
                     ifelse(score_away < score_home, mod_3$mu_xy["01"],
                            0))
      lambda = exp(alpha_i + beta_j + gamma + tau + lambda_xy)
      mu = exp(alpha_j + beta_i + tau + mu_xy)
      next_home_goal = rexp(1, rate = lambda) + minute
      next_away_goal = rexp(1, rate = mu) + minute
      next_goal = min(next_home_goal, next_away_goal)
      if(next_goal < 45) {
    ...
    U2 = rpois(1, lambda = mod_3$eta[2] + mod_3$phi[2]*(score_home + score_away) +
                 mod_3$kappa * (abs(score_home - score_away) <= 1))
    u2 = 0
    while(u2 < U2) {
      lambda_xy = ifelse(score_home > score_away, mod_3$lambda_xy["10"],
                         ifelse(score_away < score_home, mod_3$lambda_xy["01"],
