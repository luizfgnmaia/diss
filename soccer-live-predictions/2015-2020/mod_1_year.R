...
mod_1_year = list(alpha = as.vector(c(solution$getValue(alpha))),
                   ...
                   duration = duration)
names(mod_1_year$alpha) = times$Time
names(mod_1_year$beta) = times$Time
names(mod_1_year$omega) = c("lambda_x", "lambda_y", "mu_x", "mu_y",
                             "lambda_xs", "lambda_ys", "mu_xs", "mu_ys")

save(mod_1_year, file = "2015-2020/data/mod_1_year.RData")
