...
mod_0_goals = list(alpha = as.vector(c(solution$getValue(alpha))),
                   beta = as.vector(solution$getValue(beta)),
                   gamma = as.vector(solution$getValue(gamma)),
                   loglik = solution$value,
                   duration = duration)
names(mod_0_goals$alpha) = times$Time
names(mod_0_goals$beta) = times$Time

save(mod_0_goals, file = "2015-2020/data/mod_0_goals.RData")
