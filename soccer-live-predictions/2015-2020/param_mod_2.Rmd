---
title: "Parameters 2015-2020 model 2"
output: pdf_document
---

# Rates for the home and away goals

$$ \ln \lambda_k(t) = \ln \alpha_i + \ln \beta_j +  \ln \gamma_h + \mathbb{I}\{\text{half = 2}\} \ln \tau + \omega_{\lambda(x-y)}(x(t)-y(t)) + \omega_{\lambda (y^* - x^*)}(y^*(t)-x^*(t)) $$
$$ \ln \mu_k(t) = \ln \alpha_j + \ln \beta_i + \mathbb{I}\{\text{half = 2}\} \ln \tau + \omega_{\mu (y-x)}(y(t)-x(t)) + \omega_{\mu (x^* - y^*)}(x^*(t) - y^*(t)) $$
...
\item $\omega_{\lambda (x-y)}$, $\omega_{\mu (y-x)}$: parameters that measure the impact of leading in the score in the rates;
\item $\omega_{\lambda (y^*-x^*)}$ and $\omega_{\mu (x^*-y^*)}$: parameters that measure the impact of having extra players on the field.
\end{itemize}
...
Parameter = c("$\\gamma_h$", "$\\tau$", "$\\omega_{\\lambda (x-y)}$",
              "$\\omega_{\\mu (y-x)}$", "$\\omega_{\\lambda (y^*-x^*)}$",
              "$\\omega_{\\mu_(x^*-y^*)}$")
goals = tibble(Parameter, 
               Estimative = c(exp(mod_2$gamma), exp(mod_2$tau), mod_2$omega))
kable(goals, digits = 4, caption = "Goal rate parameters")
```

```{r}
Parameter = c("$A_\\lambda$", "$A_\\mu$")
reds = tibble(Parameter, Estimative = exp(mod_2$a))
...
st = tibble(Parameter, 
            Estimative = c(mod_2$eta, mod_2$rho, mod_2$kappa))
