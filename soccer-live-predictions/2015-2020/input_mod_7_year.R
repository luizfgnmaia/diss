...
par1 = matrix(0, nrow = nrow(M1_lambda), ncol = 2)
colnames(par1) = c("10", "01")
...
par2 = matrix(0, nrow = nrow(M2_lambda), ncol = 2)
colnames(par2) = c("10", "01")
for(i in 1:nrow(M1_lambda)) {
  if(x2[i] == y2[i]) {
    # do nothing
  } else if(x2[i] - y2[i] >= 1) {
...
colnames(M1_lambda)[(ncol(M1_lambda)-3):(ncol(M1_lambda)-2)] = "0"
colnames(M2_lambda)[(ncol(M2_lambda)-3):(ncol(M2_lambda)-2)] = "0"

colnames(M1_mu)[(ncol(M1_mu)-5):(ncol(M1_mu)-4)] = "0"
colnames(M2_mu)[(ncol(M2_mu)-5):(ncol(M2_mu)-4)] = "0"

save.image("2015-2020/data/input_mod_7_year.RData")
