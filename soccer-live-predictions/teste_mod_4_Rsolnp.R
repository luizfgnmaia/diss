...
log_lik <- function(par) {
  ...
  # f(t) = t
  loglambda1 = M1_lambda %*% theta + log(exp(t2_1*csi[1]) - exp(t1_1*csi[1])) - log(csi[1])
  logmu1 = M1_mu %*% theta + log(exp(t2_1*csi[2]) - exp(t1_1*csi[2])) - log(csi[2])
  loglambda2 = M2_lambda %*% theta + log(exp(t2_2*csi[1]) - exp(t1_2*csi[1])) - log(csi[1])
  logmu2 = M2_mu %*% theta + log(exp(t2_2*csi[2]) - exp(t1_2*csi[2])) - log(csi[2])
...
exp(mod_4_Rsolnp$alpha)
exp(mod_4_Rsolnp$alpha)
exp(mod_4_Rsolnp$gamma)
exp(mod_4_Rsolnp$lambda_xy)
exp(mod_4_Rsolnp$mu_xy)
mod_4_Rsolnp$eta
mod_4_Rsolnp$phi
mod_4_Rsolnp$kappa
mod_4_Rsolnp$csi