...
pred_mod_1 <- function(mod_1, n = 10^5, home_team, away_team, score_home = 0, score_away = 0, reds_home_1 = 0, reds_away_1 = 0, reds_home_2 = 0, reds_away_2 = 0, minute = 0, half = 1, end_minute = 45, end_half = 2, stoppage_time = TRUE) {
  ...
  pred <- function(home_team, away_team, score_home, score_away, reds_home_1, reds_away_1, reds_home_2, reds_away_2, minute, half, end_minute, end_half, stoppage_time) {
    ...
    if(half == 1) {
      ...
      while(minute < end_1st) {
        ...
        lambda = exp(alpha_i + beta_j + gamma + omega_lambda_x*score_home + omega_lambda_y*score_away + omega_lambda_xs*reds_home + omega_lambda_ys*reds_away)
        mu = exp(alpha_j + beta_i + omega_mu_x*score_home + omega_mu_y*score_away + omega_mu_xs*reds_home + omega_mu_ys*reds_away)
    ...
    if(end_half == 2) {
      ...
      while(minute < end_2nd) {
        ...
        lambda = exp(alpha_i + beta_j + gamma + tau + omega_lambda_x*score_home + omega_lambda_y*score_away + omega_lambda_xs*reds_home + omega_lambda_ys*reds_away)
        mu = exp(alpha_j + beta_i + tau + omega_mu_x*score_home + omega_mu_y*score_away + omega_mu_xs*reds_home + omega_mu_ys*reds_away)
        next_home_goal = rexp(1, rate = lambda) + minute
        next_away_goal = rexp(1, rate = mu) + minute
        next_home_red = t_reds_home[t_reds_home > minute][1]
        if(length(next_home_red) == 0) {
  ...
  gamma = mod_1$gamma
  tau = mod_1$tau
  omega_lambda_x = mod_1$omega["lambda_x"]
  omega_lambda_y = mod_1$omega["lambda_y"]
  omega_lambda_xs = mod_1$omega["lambda_xs"]
  omega_lambda_ys = mod_1$omega["lambda_ys"]
  omega_mu_x = mod_1$omega["mu_x"]
  omega_mu_y = mod_1$omega["mu_y"]
  omega_mu_xs = mod_1$omega["mu_xs"]
  omega_mu_ys = mod_1$omega["mu_ys"]
