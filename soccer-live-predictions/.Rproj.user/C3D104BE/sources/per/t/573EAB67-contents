...
for(i in 1:length(ind)) {
  ...
  predictions_mod_3_dc[[i]] = lst
  print(paste0(round(100*i/length(ind), 2), "%"))
}
names(predictions_mod_3_dc) = season_match

duration = Sys.time() - begin
duration # Time difference of 6.98987 hours

save(predictions_mod_3_dc, file = "weight/data/predictions_mod_3_dc.RData")
