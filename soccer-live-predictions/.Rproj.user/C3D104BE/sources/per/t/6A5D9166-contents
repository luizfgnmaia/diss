...
pred_mod_5 <- function(mod_5, n = 10^5, home_team, away_team, score_home = 0, score_away = 0, reds_home_1 = 0, reds_away_1 = 0, reds_home_2 = 0, reds_away_2 = 0, minute = 0, half = 1, end_minute = 45, end_half = 2, stoppage_time = TRUE) {
  ...
  if(!home_team %in% names(mod_5$alpha)) {
    stop("Invalid home team.")
  }
  
  if(!away_team %in% names(mod_5$alpha)) {
  ...
  alpha_i = mod_5$alpha[home_team]
  beta_i = mod_5$beta[home_team]
  alpha_j = mod_5$alpha[away_team]
  beta_j = mod_5$beta[away_team]
  gamma = mod_5$gamma
  tau = mod_5$tau
  lambda_xy = c(0, mod_5$lambda_xy)
  mu_xy = c(0, mod_5$mu_xy)
  omega_red = mod_5$omega["red"]
  A_lambda = exp(mod_5$a[1])
  A_mu = exp(mod_5$a[2])
  eta = mod_5$eta
  rho = mod_5$rho
