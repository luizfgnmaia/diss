...
pred_mod_12 <- function(mod_12, n = 10^5, home_team, away_team, score_home = 0, score_away = 0, reds_home_1 = 0, reds_away_1 = 0, reds_home_2 = 0, reds_away_2 = 0, minute = 0, half = 1, end_minute = 45, end_half = 2, stoppage_time = TRUE, value_home, value_away) {
  ...
  if(!home_team %in% names(mod_12$alpha)) {
    stop("Invalid home team.")
  }
  
  if(!away_team %in% names(mod_12$alpha)) {
  ...
  pred <- function(home_team, away_team, score_home, score_away, reds_home_1, reds_away_1, reds_home_2, reds_away_2, minute, half, end_minute, end_half, stoppage_time, value_home, value_away) {
    ...
    dif_value = log(value_home) - log(value_away)
    ...
    if(half == 1) {
      ...
      while(minute < end_1st) {
        ...
        lambda = exp(alpha_i + beta_j + gamma + omega_goal*(score_home-score_away) + omega_red*(reds_away-reds_home) + omega_value*(dif_value))
        mu = exp(alpha_j + beta_i + omega_goal*(score_away-score_home) + omega_red*(reds_home-reds_away) - omega_value*(dif_value))
    ...
    if(end_half == 2) {
      ...
      while(minute < end_2nd) {
        ...
        lambda = exp(alpha_i + beta_j + gamma + tau + omega_goal*(score_home-score_away) + omega_red*(reds_away-reds_home) + omega_value*(dif_value))
        mu = exp(alpha_j + beta_i + tau + omega_goal*(score_away-score_home) + omega_red*(reds_home-reds_away) - omega_value*(dif_value))
        next_home_goal = rexp(1, rate = lambda) + minute
        next_away_goal = rexp(1, rate = mu) + minute
        next_home_red = t_reds_home[t_reds_home > minute][1]
        if(length(next_home_red) == 0) {
  ...
  alpha_i = mod_12$alpha[home_team]
  beta_i = mod_12$beta[home_team]
  alpha_j = mod_12$alpha[away_team]
  beta_j = mod_12$beta[away_team]
  gamma = mod_12$gamma
  tau = mod_12$tau
  omega_goal = mod_12$omega["goal"]
  omega_red = mod_12$omega["red"]
  omega_value = mod_12$omega["value"]
  A_lambda = exp(mod_12$a[1])
  A_mu = exp(mod_12$a[2])
  eta = mod_12$eta
  rho = mod_12$rho
