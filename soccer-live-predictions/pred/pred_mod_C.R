# estou gerando cartões vermelhos mas não estou usando

pred_mod_C <- function(mod_C, n = 10^5, home_team, away_team, score_home = 0, score_away = 0, reds_home_1 = 0, reds_away_1 = 0, reds_home_2 = 0, reds_away_2 = 0, minute = 0, half = 1, end_minute = 45, end_half = 2, stoppage_time = TRUE, value_home, value_away) {
  ...
  if(!home_team %in% names(mod_C$alpha)) {
    stop("Invalid home team.")
  }
  
  if(!away_team %in% names(mod_C$alpha)) {
  ...
  pred <- function(home_team, away_team, score_home, score_away, reds_home_1, reds_away_1, reds_home_2, reds_away_2, minute, half, end_minute, end_half, stoppage_time, value_home, value_away) {
    ...
    if(half == 1) {
      ...
      while(minute < end_1st) {
        ...
        lambda = exp(alpha_i + beta_j + gamma + omega_goal*(score_home-score_away))
        mu = exp(alpha_j + beta_i + omega_goal*(score_away-score_home))
    ...
    if(end_half == 2) {
      ...
      while(minute < end_2nd) {
        ...
        lambda = exp(alpha_i + beta_j + gamma + tau + omega_goal*(score_home-score_away))
        mu = exp(alpha_j + beta_i + tau + omega_goal*(score_away-score_home))
        next_home_goal = rexp(1, rate = lambda) + minute
        next_away_goal = rexp(1, rate = mu) + minute
        next_home_red = t_reds_home[t_reds_home > minute][1]
        if(length(next_home_red) == 0) {
  ...
  alpha_i = mod_C$alpha[home_team]
  beta_i = mod_C$beta[home_team]
  alpha_j = mod_C$alpha[away_team]
  beta_j = mod_C$beta[away_team]
  gamma = mod_C$gamma
  tau = mod_C$tau
  omega_goal = mod_C$omega["goal"]
  # omega_red = mod_C$omega["red"]
  # omega_value = mod_C$omega["value"]
  A_lambda = exp(mod_C$a[1])
  A_mu = exp(mod_C$a[2])
  eta = mod_C$eta
  rho = mod_C$rho
