...
pred_mod_B <- function(mod_B, n = 10^5, home_team, away_team, score_home = 0, score_away = 0, reds_home_1 = 0, reds_away_1 = 0, reds_home_2 = 0, reds_away_2 = 0, minute = 0, half = 1, end_minute = 45, end_half = 2, stoppage_time = TRUE, value_home, value_away) {
  ...
  if(!home_team %in% names(mod_B$alpha)) {
    stop("Invalid home team.")
  }
  
  if(!away_team %in% names(mod_B$alpha)) {
  ...
  alpha_i = mod_B$alpha[home_team]
  beta_i = mod_B$beta[home_team]
  alpha_j = mod_B$alpha[away_team]
  beta_j = mod_B$beta[away_team]
  gamma = mod_B$gamma
  tau = mod_B$tau
  # omega_goal = mod_B$omega["goal"]
  # omega_red = mod_B$omega["red"]
  # omega_value = mod_B$omega["value"]
  A_lambda = exp(mod_B$a[1])
  A_mu = exp(mod_B$a[2])
  eta = mod_B$eta
  rho = mod_B$rho
