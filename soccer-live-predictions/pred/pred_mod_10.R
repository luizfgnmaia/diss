...
pred_mod_10 <- function(mod_10, n = 10^5, home_team, away_team, score_home = 0, score_away = 0, reds_home_1 = 0, reds_away_1 = 0, reds_home_2 = 0, reds_away_2 = 0, minute = 0, half = 1, end_minute = 45, end_half = 2, stoppage_time = TRUE, value_home, value_away) {
  ...
  if(!home_team %in% names(mod_10$alpha)) {
    stop("Invalid home team.")
  }
  
  if(!away_team %in% names(mod_10$alpha)) {
  ...
  alpha_i = mod_10$alpha[home_team]
  beta_i = mod_10$beta[home_team]
  alpha_j = mod_10$alpha[away_team]
  beta_j = mod_10$beta[away_team]
  gamma = mod_10$gamma
  tau = mod_10$tau
  lambda_xy = c(0, mod_10$omega_xy)
  mu_xy = c(0, rev(mod_10$omega_xy))
  omega_red = mod_10$omega["red"]
  omega_value = mod_10$omega["value"]
  A_lambda = exp(mod_10$a[1])
  A_mu = exp(mod_10$a[2])
  eta = mod_10$eta
  rho = mod_10$rho
