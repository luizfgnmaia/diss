...
fit_mod_12 <- function(date, csi) {
  ...
  tau = Variable(1)
  omega = Variable(3)
  theta = vstack(alpha, beta, gamma, tau, omega[1], omega[1], omega[2], omega[2], omega[3])
  ...
  mod_12 = list(alpha = as.vector(c(solution$getValue(alpha))),
  ...
  names(mod_12$alpha) = times$Time
  names(mod_12$beta) = times$Time
  names(mod_12$omega) = c("goal", "red", "value")
  names(mod_12$a) = c("lambda", "mu")
...
fit_mod_12_dates <- function(csi) {
  ...
  # lapply(dates, function(x) fit_mod_12(x, csi))
  ret = list()
  for(i in 1:length(dates)) { 
    ret[[i]] = fit_mod_12(dates[i], csi)
