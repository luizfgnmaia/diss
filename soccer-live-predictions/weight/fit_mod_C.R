...
fit_mod_C <- function(date, csi) {
  ...
  mod_C = list(alpha = as.vector(c(solution$getValue(alpha))),
  ...
  names(mod_C$alpha) = times$Time
  names(mod_C$beta) = times$Time
  names(mod_C$omega) = "goal"
  names(mod_C$a) = c("lambda", "mu")
...
fit_mod_C_dates <- function(csi) {
  ...
  # lapply(dates, function(x) fit_mod_C(x, csi))
  ret = list()
  for(i in 1:length(dates)) { 
    ret[[i]] = fit_mod_C(dates[i], csi)
