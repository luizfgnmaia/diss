...
fit_mod_9 <- function(date, csi) {
  ...
  mod_9 = list(alpha = as.vector(c(solution$getValue(alpha))),
  ...
  names(mod_9$alpha) = times$Time
  names(mod_9$beta) = times$Time
  names(mod_9$omega_xy) = c("ahead", "behind")
  names(mod_9$omega) = c("red", "value")
  names(mod_9$a) = c("lambda", "mu")
...
fit_mod_9_dates <- function(csi) {
  ...
  # lapply(dates, function(x) fit_mod_9(x, csi))
  ret = list()
  for(i in 1:length(dates)) { 
    ret[[i]] = fit_mod_9(dates[i], csi)
