...
fit_mod_8 <- function(date, csi) {
  ...
  mod_8 = list(alpha = as.vector(c(solution$getValue(alpha))),
  ...
  names(mod_8$alpha) = times$Time
  names(mod_8$beta) = times$Time
  names(mod_8$omega_xy) = c("ahead", "behind")
  names(mod_8$omega) = "red"
  names(mod_8$a) = c("lambda", "mu")
...
fit_mod_8_dates <- function(csi) {
  ...
  # lapply(dates, function(x) fit_mod_8(x, csi))
  ret = list()
  for(i in 1:length(dates)) { 
    ret[[i]] = fit_mod_8(dates[i], csi)
