...
fit_mod_B <- function(date, csi) {
  ...
  mod_B = list(alpha = as.vector(c(solution$getValue(alpha))),
  ...
  names(mod_B$alpha) = times$Time
  names(mod_B$beta) = times$Time
  names(mod_B$a) = c("lambda", "mu")
...
fit_mod_B_dates <- function(csi) {
  ...
  # lapply(dates, function(x) fit_mod_B(x, csi))
  ret = list()
  for(i in 1:length(dates)) { 
    ret[[i]] = fit_mod_B(dates[i], csi)
